...
def clean_response(reply):
    ...
    reply = re.sub(r"^(Sure|Here|Yes|Certainly|Of course|Absolutely|Okay)[,\s]*(here's|here is|this is|we have|the answer).*?:\s*", "", reply, flags=re.IGNORECASE)
...
6. If the question asks about high monthly expenses, explain that the system may recommend a smaller loan amount to avoid default, and that high-risk applicants may be asked to provide an eligible guarantor before final loan approval.
